...
export default function monthlySummaryNew(
  habits: Habit[],
        ...
        habits.forEach((h) => {
          if (s[h.keyName]) {
            switch (h.dataType) {
              ...
              default:
                throw Error(
                  "Habit is not boolean or number, is: " + h.dataType,
                );
            }
          }
        });
    ...
    sections.push(
      <View key={"monthlySummaryTitle"}>
        <Text style={styles.homeScreenSubtitle}>
          {monthNames[today.getMonth()] + " Summary"}
        </Text>
      </View>,
    );
    habits.forEach((h) => {
        ...
        sections.push(
          <View key={h.keyName + "MonthSummary"}>
            <Text style={styles.regularText}>
              {h.prettyPrint} : {count}/{currentMonthKeys.length}{" "}
              {currentMonthKeys.length === 1 ? "day" : "days"}
            </Text>
          </View>,
        );
