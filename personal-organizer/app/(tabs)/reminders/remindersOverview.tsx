...
export default function RemindersPage() {
  ...
  const [dailyReminder, setDailyReminder] =
    useMMKVObject<boolean>("dailyReminder");
    ...
    if (reminderArray) {
      ...
      for (let i = 0; i < reminderArray.length; i++) {
        if (activeReminders.includes(reminderArray[i].notificationID)) {
          ...
          newSections.push(
            <View
              key={reminderArray[i].notificationID}
              style={styles.reminderPageContainer}
            >
              <Link
                href={{
                  pathname: "/reminders/[reminder]",
                  params: { reminder: reminderArray[i].notificationID },
                }}
              >
                <Text style={reminderStyles.reminderOverviewHyperlink}>
                  {reminderArray[i].title}
                </Text>
              </Link>
              <Text style={reminderStyles.overviewInfo}>
                {getKeyReminderInfo(reminderArray[i])}
              </Text>
              <Ionicons.Button
                style={reminderStyles.cancelButton}
                name="ban-sharp"
                size={14}
                color="#CF6679"
                ...
                onPress={() => {
                  deleteReminder(
                    reminderArray[i].notificationID,
                    reminderArray[i].title,
                  );
                }}
              />
      ...
      <ScrollView>
        <Surface style={styles.homeScreenSurface} elevation={1}>
          ...
          <View key="newReminderButton" style={styles.buttonContainer}>
            <Pressable
              onPress={() => router.push("/reminders/newReminderForm")}
              style={() => [
                {
  ...
  if (r.repeats !== "Once") {
    if (r.repeats === "Yearly") {
      returnString =
        returnString +
        "Yearly on " +
        (r.repeatDetails.month + 1) +
        "/" +
        r.repeatDetails.dayOfMonth;
    ...
    returnString =
      returnString +
      " at " +
      (r.repeatDetails.hour > 12
        ? r.repeatDetails.hour - 12
        : r.repeatDetails.hour) +
      ":" +
      (r.repeatDetails.minute < 10 ? "0" : "") +
      r.repeatDetails.minute +
      " " +
