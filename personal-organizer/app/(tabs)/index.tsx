...
export default function HomeScreen() {
  ...
  const [todaysKeyIndex, setTodaysKeyIndex] =
    useMMKVObject<string>("todaysKeyIndex");
  let today = new Date();
  ...
  useEffect(() => {
    if (habits && submissions && todaysKeyIndex) {
      ...
      let sundayKey = generateDifferentDaysKey(
        todaysKeyIndex,
        daysUntilSunday[new Date(todaysKeyIndex).getUTCDay()] + 7 * mult,
      ); //applies multiplier for previous weeks
