...
export default function Form() {
  ...
  //Personal Care
  const [morningBrush, setMorningBrush] = useState<boolean>(
    data ? data.morningBrush : false,
  );
  const [nightBrush, setNightBrush] = useState<boolean>(
    data ? data.nightBrush : false,
  );
  const [usedMouthwash, setUsedMouthwash] = useState<boolean>(
    data ? data.usedMouthwash : false,
  );
  const [washedFace, setWashedFace] = useState<boolean>(
    data ? data.washedFace : false,
  );
  const [usedExfoliator, setUsedExfoliator] = useState<boolean>(
    data ? data.usedExfoliator : false,
  );
  const [showered, setShowered] = useState<boolean>(
    data ? data.showered : false,
  );
  const [tookMedicine, setTookMedicine] = useState<boolean>(
    data ? data.tookMedicine : false,
  );

  //Workout
  const [minutesBiked, setMinutesBiked] = useState<string>(
    data ? data.minutesBiked.toString() : "0",
  );
  const [situpsDone, setSitupsDone] = useState<string>(
    data ? data.situpsDone.toString() : "0",
  );
  const [pushupsDone, setPushupsDone] = useState<string>(
    data ? data.pushupsDone.toString() : "0",
  );

  //Productivity
  const [leetcodeMinutes, setLeetcodeMinutes] = useState<string>(
    data ? data.leetcodeMinutes.toString() : "0",
  );
  const [personalProjectMinutes, setPersonalProjectMinutes] = useState<string>(
    data ? data.personalProjectMinutes.toString() : "0",
  );
  const [artMinutes, setArtMinutes] = useState<string>(
    data ? data.artMinutes.toString() : "0",
  );
